...
\begin{enumerate}
  ...
  allowing one to write ALE code by just importing the corresponding library. This is a very
...
The ALE provides optional functionalities that are not activated by default, but if chosen the user is 
...
% TODO(mgbellemare): describe -DUSE_SDL, -DUSE_RLGLUE instead.
...
% TODO(mgbellemare): pass filename
...
% TODO(mgbellemare): include filenames of examples
\begin{verbatim}
...
% TODO(mgbellemare): p=0.25 was chosen because it was the highest value for which human play-testers were unable to detect any delay or control lag as a result of the action repeats.
...
% TODO(mgbellemare): mention thread safety, or at the very least no conflict with rand/srand
