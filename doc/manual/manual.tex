...
\begin{itemize}
\item Where are the ROMS?

 We do not provide them due to copyrights.

\item unsuported ROM file

the rom file should be specified as the last argument on the command line. also it has to be lower 
case. Maybe it is too late but it is probably because you are not entering the right name for thebinary file. Each game has to be entered with a specific name which is specified at the header file (hpp) of the game. You can find these header files at games/supported if I am not wrong. Hope this solves the problem.

\item sdl not found.

add the other path.

\item extract lives signal.

now we support it.

\item pixel do not have value greater than 127.

I hope have already solved your problem, but I am going to answer you anyway. When you get the pixel value, it is a byte, but you want 7 bits. You assumed the most significative bit was always zero, but in fact, what is always zero is the least significative bit. Therefore, you have to shift the byte >> 1 to obtain the colour value.

\item usr/bin/ld: cannot find -lrlagent (I manually search inside the libs and includes of rlglue and “lrlagent” it not even there and the directory “ld” doesn’t exist in /usr/bin)
